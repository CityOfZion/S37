...
web (ChatPage component at /payments)
...
- **Auth model is stateless JWT** (Bearer token, not cookies). The OAuth callback signs a 7-day HS256 JWT and hands it to the browser in a URL fragment; the SPA stores it in `localStorage` (`fractapay.token`) and sends it as `Authorization: Bearer …` on every API call. There is no server-side session store — `requireAuth` verifies the signature in-process. The cookie scheme (signed `fractapay_session` + `Session` table) was removed. `@fastify/cookie` is still registered but only because `@fastify/oauth2` uses it for the `state`/PKCE round-trip during the redirect dance — it is not used for app auth.
...
- **`src/routes/auth-route.ts`** — `GET /auth/google` is registered automatically by `@fastify/oauth2` (as `startRedirectPath`) and kicks off the authorization-code + PKCE flow. `GET /auth/google/callback` exchanges the code for tokens, fetches the userinfo, upserts the user, signs the JWT via `reply.jwtSign`, sets `Cache-Control: no-store` (so the token-bearing redirect isn't cached/logged), and redirects to `WEB_LOGIN_SUCCESS_URL#token=<jwt>` built with `new URL` (or `WEB_LOGIN_FAILURE_URL` on any error). `POST /auth/logout` (uses `optionalAuth`) is a no-op success (stateless — the client wipes its own `localStorage`; there is nothing to revoke server-side). `GET /auth/me` requires `requireAuth` and returns `{ user: TUser }`. `POST /auth/onboarding` requires `requireAuth`, takes `TCompleteOnboardingPayload { companyName }` (400 `INVALID_PAYLOAD` if blank), calls `markOnboardingCompleted(userId, companyName)` to stamp `onboardingCompletedAt = now`, and returns the refreshed `{ user: TUser }`. `POST /auth/signup` is a placeholder that replies `501 NOT_IMPLEMENTED` (Google OAuth is the only real path for now).

### Web internals

- **Routing** — TanStack Router (`src/router/index.tsx`). The root route is a bare `<Outlet />`. A child `authRoute` (`id: 'auth'`) carries the `RootLayout` (sidebar) and a `beforeLoad` guard that calls `fetchCurrentUser()` (`GET /auth/me` via the `server` axios instance) → redirect to `/login` if no user, to `/onboarding` if `!user.onboardingCompletedAt`. Its children are the authed pages: `/payments` (ChatPage — AI payment chat), `/destinations` (DestinationsPage — recipients CRUD), `/payment/$orderId` (PaymentPage — order status polling); the index `/` `beforeLoad`-redirects to `/payments`. Sibling top-level routes (no layout): `/login` (LoginPage — redirects to `/payments` if already onboarded), `/onboarding` (OnboardingPage — redirects to `/login` if no user, `/payments` if already onboarded), and a catch-all `$` that redirects to `/payments`. `basepath` is `BASE_PATH` (`/S37` in prod, `undefined` in dev — see `src/constants`).
