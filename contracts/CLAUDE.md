...
- **Storage.** Persistent storage holds everything: `Admin`, `NextAgreementId`, `Agreement(id)`, `PayerAgreements(payer)`, `PaymentHistory(payer)` — all extended to TTL `535_000` (threshold `100`) on every read/write. The instance TTL is still bumped on every mutating entry point via `extend_instance_ttl`, so the contract instance + Wasm are not archived (instance storage itself holds no data now).
