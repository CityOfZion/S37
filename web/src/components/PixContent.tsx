...
export const PixContent = ({
  ...
  const { data: paymentData } = usePaymentQuery(paymentId ?? '', { refetch })

  useEffect(() => {
    if (!paymentData || paidRef.current || !onPaid) return

    if (PAYMENT_PAID_STATUSES.has(paymentData.status)) {
  ...
  return (
    <div className="space-y-4 mt-8">
      ...
      {paymentId && (!currentStatus || !PAYMENT_PAID_STATUSES.has(currentStatus)) && (
