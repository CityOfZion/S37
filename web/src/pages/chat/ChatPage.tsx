...
export const ChatPage = () => {
  ...
  const { t: tDestinations } = useTranslation('pages', { keyPrefix: 'destinations' })
  ...
  const { setToken, token: paymentToken, address, setAddress } = useDataStore()
  const { id } = useParams({ strict: false })
  const { data: paymentsData } = usePaymentsQuery()
  const { data: paymentData } = usePaymentQuery(id || '', {
    refetch: false,
  })

  const dateOptions: Intl.DateTimeFormatOptions = { day: 'numeric', month: 'long', year: 'numeric' }

  const breadcrumbLabel = paymentData
    ? paymentData.amount
      ? tCommon('paymentTitle', {
          count: paymentData.destinations.length,
          date: new Date(paymentData.createdAt).toLocaleDateString(language, dateOptions),
          amount: StringHelper.formatCurrencyAmount(paymentData.amount, paymentData.token),
          recipients: paymentData.destinations.length,
        })
  ...
  useBreadcrumb(
    breadcrumbLabel
      ? [{ label: t('title'), to: '/chat' }, { label: breadcrumbLabel }]
      : [{ label: t('title') }]
  )
  ...
  const destinationPendingTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  ...
  const isLoading =
    chatMutation.isPending || addDestinationMutation.isPending || isDestinationPending
  const isLoadingRef = useRef(false)
  const inputDisabled = isLoading || orderExecuted || isViewing
  ...
  useEffect(() => {
    if (!isLoading && !(!id && hasUserMessage)) return
  ...
  useEffect(() => {
    ...
    resetChat()
    setMessages(
      paymentData.messages.map(message => ({
        id: message.id,
        role: message.role,
        text: message.text,
        type: 'text' as const,
  ...
  }, [id, paymentData?.id]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const sendMessage = useCallback(
      ...
      chatMutation.mutate(
        ...
        },
        {
          onSuccess: response => {
            ...
            const text = isBelowMinimum
              ? t('minimumAmountError', {
                  amount: StringHelper.formatCurrencyAmount(
                    MINIMUM_PAYMENT_AMOUNT.toFixed(),
                    paymentToken
                  ),
                })
              : response.error
                ? response.error === EErrorCode.NO_PAYMENTS_FOUND
            ...
            if (response.action === 'EXECUTE' && !isBelowMinimum) startExecution(destinations)
            if (response.action === 'CLEAR') resetChat()

            if (response.action === 'CREATE_DESTINATION' && response.newDestination) {
              const destination = response.newDestination
              const pixKey = InputHelper.normalizePixKey(destination.pixKey, destination.pixKeyType)
              ...
              addDestinationMutation.mutate(
                ...
                {
                  onSuccess: () => {
                    if (destinationPendingTimeoutRef.current) {
                      clearTimeout(destinationPendingTimeoutRef.current)
                    }

                    destinationPendingTimeoutRef.current = setTimeout(() => {
                      setIsDestinationPending(false)

                      addMessage({
                        id: uuid.v4(),
                        role: 'ASSISTANT',
                        text: tDestinations('addSuccess'),
                        type: 'text',
                        timestamp: new Date().toJSON(),
                      })
                    }, 2000)
                  ...
                  onError: error => {
                    ...
                    const errorText = tDestinations(code || 'saveError', {
                      defaultValue: tDestinations('saveError'),
                    })
    ...
    [
      ...
      setIsDestinationPending,
      destinationPendingTimeoutRef,
      tDestinations,
      t,
    ]
  )
  ...
  return (
    <div className="flex h-full">
      ...
      <div
        className="relative flex flex-col flex-1 min-w-0 h-full overflow-y-auto"
      ...
      >
        ...
        {paymentData?.externalId && id && (
          <div className="sticky top-0 z-10 flex justify-center pt-6 pointer-events-none">
        ...
        <div className="flex-1 shrink-0 px-4 py-6 space-y-4 max-w-3xl mx-auto w-full">
