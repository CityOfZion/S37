...
import { useSignTransactionsMutation } from '../../hooks/use-sign-transactions-mutation'
...
export const PaymentPage = () => {
  ...
  const signTransactionsMutation = useSignTransactionsMutation()
  const hasSignedTransactionsRef = useRef(false)

  useEffect(() => {
    hasSignedTransactionsRef.current = false
  }, [paymentId])
  ...
  useEffect(() => {
    if (hasSignedTransactionsRef.current || !paymentData || paymentData.status !== 'PROCESSING')
      return

    const hasPendingTransactions = paymentData.destinations.some(
      destination => destination.transactionData && !destination.completed
    )

    if (!hasPendingTransactions) return

    hasSignedTransactionsRef.current = true

    signTransactionsMutation.mutate(
      paymentData.destinations
        .filter(destination => !!destination.transactionData && !destination.completed)
        .map(({ transactionData }) => transactionData) as string[]
    )
  }, [paymentData, signTransactionsMutation])
  ...
  const isPixPending = payment?.pix && payment.status === 'CREATED'
        ...
        {isError ? (
          ...
          <>
            ...
            {signTransactionsMutation.isPending && (
              <LoadingBanner>{t('signingTransactions')}</LoadingBanner>
            )}

            {signTransactionsMutation.isError && (
              <ErrorBanner>{t('signingTransactionsError')}</ErrorBanner>
            )}
            ...
            {payment && !isLoading && (
              <div className="space-y-4">
                ...
                {(isPixPending || payment.destinations.length > 0) && (
                      ...
                      <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm p-6 space-y-3 w-full lg:flex-1">
                        ...
                        <ul className="divide-y divide-neutral-100">
                          {payment.destinations.map(destination => (
                            <li
                              key={destination.id}
                              className="flex flex-col py-2.5 text-sm first:pt-0 last:pb-0"
                            >
                              <div className="flex items-center justify-between gap-4">
                                ...
                                <div className="shrink-0 text-right">
                                  ...
                                  <p className="text-neutral-400 text-xs">
                                    {DestinationsHelper.formatPercentage(destination.percentage)}
                                  </p>
                                </div>
                              </div>
                              {destination.transactionHash && destination.transactionUrl && (
                                <div className="flex items-center gap-1 -mt-0.75">
                                  ...
                                  <CopyButton value={destination.transactionHash} />
                                </div>
                              )}
                            </li>
                          ))}
